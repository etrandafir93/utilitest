 ...
 *       factory.
 *   <li>{@link #argThat(Consumer)} - Creates a Mockito argument matcher using a generic AssertJ
 *       `ObjectAssert`.
...
public class MockitoAndAssertJ {
     ...
     */
    public static <T> T argThat(Consumer<ObjectAssert<T>> assertion) {
    ...
    private static <T, ASSERT extends AbstractAssert<?, ?>> T argThat(
    ...
    public static class Arg<ASSERT extends AbstractAssert<?, ?>> {
        ...
        public <T> T that(Consumer<ASSERT> assertion) {
            return argThat(assertionType, assertion);
        }
    }
}
